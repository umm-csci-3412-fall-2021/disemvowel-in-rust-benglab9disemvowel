...
fn main() {
    ...
    let in_file = &args.get(1).expect("Not enough arguments");
    let out_file = &args.get(2).expect("Not enough arguments");

    let s = String::from(read_file(Path::new(in_file)));
    ...
    write_file(Path::new(out_file), &s_disemvowel)
}
